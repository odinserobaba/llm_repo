...
## 🎬 Сториборд: жизненный цикл модели с LoRA/PEFT

Чтобы увидеть картину целиком — от базовой модели до продакшена — можно открыть Excalidraw‑диаграмму:

- base LLM (общий переводчик),
- добавление LoRA‑адаптера (0.05% весов, A,B матрицы),
- обучение по инструкциям (SFT) только этих параметров,
- адаптер как отдельный файл, который можно хранить и переключать,
- две ветки деплоя: base+адаптер (PEFT) и merge в один чекпоинт.

Диаграмма лежит в файле:

- [[07_finetuning_lora/model_lifecycle_storyboard.excalidraw|Жизненный цикл модели с LoRA/PEFT (Excalidraw)]]
