## Graph: Information Extraction (IE)

Хаб по извлечению информации: NER, relation extraction, хранение и анализ IE‑результатов.

---

## Базовые модули и HW

- [[09_module_ie/README|Модуль 9 — Information Extraction]]
- [[09_hw_ie_extraction/README|HW — IE Extraction]]

---

## NER и IE‑пайплайны

- [[09_module_ie/01_ner_spacy_natasha_hf|NER: spaCy, Natasha, HF модели]]
- [[09_module_ie/02_information_extraction|Пайплайны IE, схемы аннотации]]
- [[09_module_ie/03_storage_visualization|Хранилище результатов IE, визуализация]]

---

## Домашка и разбор результатов

- [[09_hw_ie_extraction/ie_extraction_hw|Основной ноутбук HW по IE]]
- [[09_hw_ie_extraction/ie_extraction_hw_result.ipynb|Результаты 1 (notebook)]]  <!-- останется как ссылка на файл -->
- [[09_hw_ie_extraction/ie_extraction_hw_result_2.ipynb|Результаты 2 (notebook)]]
- [[09_hw_ie_extraction/ie_extraction_hw_rel2.ipynb|rel2 эксперимент]]
- [[09_hw_ie_extraction/РЕЗУЛЬТАТЫ_rel2_НАГЛЯДНО|Результаты rel2 наглядно]]
- [[09_hw_ie_extraction/АНАЛИЗ_РЕЗУЛЬТАТОВ|Анализ результатов IE]]

---

## Промптинг для IE

- [[09_hw_ie_extraction/prompt_cognitive_designer|Промпты IE: когнитивный дизайнер]]
- [[graph_Prompting|Graph: Промптинг и паттерны]] — общие техники, применимые к IE

---

## Связи с другими хабами

- [[graph_RAG|Graph: RAG и поиск]] — IE как источник структурированных фактов для RAG/GraphRAG
- [[graph_Agents_LangGraph|Graph: Агенты и LangGraph]] — агенты, управляющие IE‑пайплайном
- [[00_Home|← Назад в Home]]

---

**Теги:** #ie #ner #extraction #отношения #аннотация

